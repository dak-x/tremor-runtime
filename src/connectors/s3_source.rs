...
async fn fetch_keys_task(
    ...
    // FIXME: DONOT DO THIS, DONE ONLY FOR TESTING WHETHER THE ARCHITECTURE WORKS OR NOT.
    // THE CONTINUATION TOKEN MIGHT SURELY HAS LIMITED LIFETIME.
    ...
    'outer: loop {
        ...
        let cont_token = resp.next_continuation_token;
        resp = client
            .list_objects_v2()
            .bucket(bucket.clone())
            .set_prefix(prefix.clone())
            .set_delimiter(delim.clone())
            .set_continuation_token(cont_token)
            .send()
            .await?;
    }
